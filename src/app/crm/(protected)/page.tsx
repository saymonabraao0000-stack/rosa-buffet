  ...
  return (
    <div>
      ...
      <div className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-2">
        <section>
          <h2 className="mb-4 font-display text-xl text-ink">Leads por status</h2>
          <ul className="flex flex-col gap-2 rounded-xl border border-ink/10 bg-white/50 p-4">
            {Object.entries(stats.porStatus).map(([status, count]) => (
              <li key={status} className="flex items-center justify-between text-sm">
                <span className="text-gray-dark">{STATUS_LABELS[status] ?? status}</span>
        ...
        <section>
          <h2 className="mb-4 font-display text-xl text-ink">Próximos eventos fechados</h2>
          {stats.proximosEventos.length === 0 ? (
            <p className="text-sm text-gray-dark">Nenhum evento confirmado por enquanto.</p>
          ) : (
            <ul className="flex flex-col gap-2 rounded-xl border border-ink/10 bg-white/50 p-4">
              {stats.proximosEventos.map((evento) => (
                <li key={evento.id} className="flex items-center justify-between text-sm">
                  <Link
                    ...
                    className="focus-gold text-ink transition-colors hover:text-gold"
                  >
                    {evento.nome}
                  </Link>
...
function StatTile({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-xl border border-ink/10 bg-white/50 p-4">
      <span className="block text-xs font-semibold uppercase tracking-wide text-gray-dark">
        {label}
      </span>
      <span className="mt-2 block font-display text-3xl text-ink">{value}</span>
    </div>
  );
}
