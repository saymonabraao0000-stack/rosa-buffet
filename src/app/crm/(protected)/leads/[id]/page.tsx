  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl text-ink">{lead.nome}</h1>
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <section className="rounded-xl border border-ink/10 bg-white/50 p-5">
            <h2 className="mb-4 font-display text-lg text-ink">Respostas do simulador</h2>
            <dl className="grid grid-cols-1 gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
              ...
              <div className="col-span-full">
                <dt className="text-xs font-semibold uppercase tracking-wide text-gray-dark">Opcionais</dt>
          ...
          <section className="mt-6 rounded-xl border border-ink/10 bg-white/50 p-5">
            <h2 className="mb-4 font-display text-lg text-ink">Anotações</h2>
            <NoteForm leadId={lead.id} />
            <ul className="mt-5 flex flex-col gap-4">
              {notes.map((note) => (
                <li key={note.id} className="border-t border-ink/10 pt-3 text-sm">
              ...
              {notes.length === 0 && (
                <li className="text-sm text-gray-dark">Nenhuma anotação ainda.</li>
              )}
            </ul>
          </section>
        </div>
        ...
        <aside className="flex flex-col gap-6">
          ...
          <section className="rounded-xl border border-ink/10 bg-white/50 p-5">
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-dark">
              Reserva
            </h2>
...
function Field({ label, value }: { label: string; value: string | undefined }) {
  return (
    <div>
      <dt className="text-xs font-semibold uppercase tracking-wide text-gray-dark">{label}</dt>
