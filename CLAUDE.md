...
- **Domínio definitivo: `rosabuffeteventos.com.br`** (sem o "e" do meio; escolhido pela família da dona em 2026-09-23 no lugar de `rosabuffetmanaus.com.br`; ainda não registrado). Titular: CNPJ 55.500.499/0001-98 (MEI de **Rosilene Moreira de Paula**, a dona, ativo desde 2024-06-12 — conferido na Receita); e-mail do cadastro rosabuffet26@gmail.com; paga por Pix, 1 ano por vez. Ela não tinha conta no Registro.br. A dona é a mãe da Allyne, namorada do Saymon; o contato com ela passa pela Allyne. **Não usar `rosabuffet.com.br`**: ele existe desde 2019, mas pertence a OUTRO Rosa Buffet (titular Roselina Soares de Oliveira Barbeito, que não é a dona deste buffet de Manaus). O registro novo deve sair no nome da dona (CPF ou CNPJ), com a Ervexa como contato técnico. `siteConfig.url` em [site-config.ts:13](src/lib/site-config.ts#L13) e `email` ainda apontam para `rosabuffet.com.br` — trocar. Esse valor alimenta `canonical`, `sitemap.ts`, `robots.ts` e as meta tags Open Graph/Twitter em `layout.tsx`. Hospedagem também em aberto: o plano Hobby da Vercel não permite uso comercial (opções: Vercel Pro ou Cloudflare Workers + OpenNext).
