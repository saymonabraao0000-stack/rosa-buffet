...
- Next.js `16.2.10` (App Router, React Server Components, Server Actions)
...
proxy.ts                    # (raiz do projeto) protege /crm/** — Next 16 renomeou middleware.ts para proxy.ts
db/
...
- [src/lib/crm/actions.ts](src/lib/crm/actions.ts) — cada uma chama `requireSession()` internamente. Isso é necessário mesmo com o `proxy.ts` bloqueando `/crm/**`: a própria documentação do Next.js avisa que Server Actions não herdam a proteção da rota onde são definidas (são tratadas como POST pra aquela rota, então um matcher que exclua a rota também abre a Server Action sem querer) — por isso a checagem é duplicada dentro de cada ação sensível, não só no proxy.

**Autenticação do `/crm`**: senha única compartilhada (não é login por pessoa — decisão explícita do dono, que quis algo simples). `CRM_PASSWORD` fica direto como env var na Vercel (não hasheada — é um segredo já protegido pelo cofre de env vars da Vercel, adequado pro nível de ameaça de uma ferramenta interna pequena). [src/lib/crm/session.ts](src/lib/crm/session.ts) assina um cookie de sessão com HMAC-SHA256 (`SESSION_SECRET`, outra env var) usando só `node:crypto` — nenhuma lib de auth foi adicionada. [proxy.ts](proxy.ts) na raiz do projeto bloqueia `/crm/**` (exceto `/crm/login`) checando esse cookie.

**`proxy.ts`, não `middleware.ts`**: o Next 16.0.0 renomeou o arquivo — `middleware.ts`/função `middleware` estão deprecados, o novo é `proxy.ts`/função `proxy`, mesma `config.matcher`. Proxy roda em runtime Node.js por padrão desde a v16 (antes era Edge), por isso dá pra usar `node:crypto` (`createHmac`, `timingSafeEqual`) sem gambiarra de Web Crypto.
